...
def get_one_table(fin):
    ...
    otv=pd.DataFrame(Df.iloc[:,[3,6]].values,index=my_index,columns=[str('Date'),fin[1]])
...
while i < len(sys.argv):
    hash2name.append((sys.argv[i], sys.argv[i+1]))
    i += 2

get_otchet_Both(hash2name)
